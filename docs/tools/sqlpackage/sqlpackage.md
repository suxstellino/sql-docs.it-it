...
|AzureSharedAccessSignatureToken = (stringa)| | x | | | | |
|AzureStorageBlobEndpoint = (stringa)| x | x | | | | |
|AzureStorageContainer = (stringa)| x | x | | | | |
|AzureStorageKey = (stringa)| x | x | | | | |
|AzureStorageRootPath = (stringa)| x | x | | | | |
...
|DisableIndexesForDataPhase = (valore BOOLEANo ' true ')| | | | x | | |
...
|RebuildIndexesOfflineForDataPhase = (valore BOOLEANo ' false ')| | | | x | | |
