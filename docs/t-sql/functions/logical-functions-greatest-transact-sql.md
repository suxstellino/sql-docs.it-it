---
description: Funzioni logiche-maggiore (Transact-SQL)
title: MAGGIORE (Transact-SQL)
ms.custom: ''
ms.date: 04/09/2021
ms.prod: sql
...
ms.locfileid: "107299409"
---
# <a name="logical-functions---greatest-transact-sql"></a>Funzioni logiche-maggiore (Transact-SQL)
 ...
 *expression1, expressionn*  
 Elenco di espressioni delimitate da virgole di qualsiasi tipo di dati confrontabile. La  `GREATEST`   funzione richiede almeno un argomento e non supporta più di 254 argomenti.  
...
## <a name="remarks"></a>Commenti  
 ...
 Se uno o più argomenti non sono `NULL` , gli `NULL` argomenti verranno ignorati durante il confronto. Se tutti gli argomenti sono `NULL` , `GREATEST` restituirà `NULL` . 
 ...
 I tipi seguenti **non** sono supportati per il confronto in `GREATEST` : **varchar (max), varbinary (max) o nvarchar (max) che superano 8.000 byte, cursore, geometria, geografia, immagine, tipi definiti dall'utente non ordinati per byte, ntext, Table, text** e **XML**. 
...
### <a name="c-simple-example-with-table"></a>C. Esempio semplice con tabella
 ...
 In questo esempio viene restituito il valore massimo da un elenco di argomenti di colonna e i valori vengono ignorati `NULL` durante il confronto. 
...
### <a name="d-using-greatest-with-local-variables"></a>D. Uso `GREATEST` di con variabili locali
 ...
 In questo esempio viene usato `GREATEST` per determinare il valore massimo di un elenco di variabili locali all'interno del predicato di una `WHERE` clausola. 
...
### <a name="e-using-greatest-with-columns-constants-and-variables"></a>E. Utilizzo `GREATEST` di con colonne, costanti e variabili

 In questo esempio viene usato `GREATEST` per determinare il valore massimo di un elenco che include colonne, costanti e variabili. 
...
## <a name="see-also"></a>Vedere anche  
 [&#41;Transact-SQL meno &#40;](../../t-sql/functions/logical-functions-least-transact-sql.md)  
