...
description: Visualizzare lo schema XSD con annotazioni utilizzato negli esempi di questa sezione per illustrare le query XPath in SQLXML 4.0.
ms.date: 03/16/2017
ms.prod: sql
...
-   Creare un modello contenente una query XPath. Specificare nel modello lo schema di mapping sul quale eseguire la query XPath. In questo caso, lo schema di mapping deve essere archiviato nella directory (o in una delle relative sottodirectory, nel qual caso viene specificato un percorso relativo come valore dell'attributo **mapping-schema** nel modello) associato al file modello.  
  
-   Creare un'applicazione di test che utilizza le estensioni SQLXML per eseguire le query con ADO. Per altre informazioni, vedere [Utilizzo di ADO per l'esecuzione di query SQLXML 4.0.](../../../relational-databases/sqlxml/using-ado-to-execute-sqlxml-4-0-queries.md)  
